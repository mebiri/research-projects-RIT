...
#import lalframe #unused import
import lal
import functools
#import itertools #unused except in one instance, reimported at that point
...
#CIP faster args (can be ignored otherwise)
parser.add_argument("--save-hyperfile-only",action="store_true",help="Use to only save MARG-X-X+annotation.dat file, the only one used by HyperPipe (reduces number of output files)")
parser.add_argument("--chunk-save",action="store_true",help="Use to save single-line output files as one file for multiple EOS lines (reduces number of output files)")
...
if opts.sampler_method == "adaptive_cartesian_gpu" or "AC" in port_list or "adaptive_cartesian_gpu" in port_list:
    try:
        ...
        sampler_check += 1
        if feedback: print(" Imported sampler: adaptive_cartesian_gpu/AC")
    except:
...
if opts.sampler_method == 'portfolio' or sampler_check == 0:
    try:
        ...
        print(" No mcsamplerPortfolio ") #---------------usual print - so usually False

if feedback: print(" -- FIT METHOD:",opts.fit_method," -- ")
#from sklearn.preprocessing import PolynomialFeatures #unused import
if opts.fit_method == "polynomial":
    try: #only used for opts.fit_method == "polynomial"
        import RIFT.misc.ModifiedScikitFit as msf  # altenative polynomialFeatures
        if feedback: print(" Imported: RIFT ModifiedScikitFit")
    except:
        print(" - Failed ModifiedScikitFit : No polynomial fits - ") #don't get this output
    from sklearn import linear_model #also only for fit polynomial
    if feedback: print(" Imported: sklearn")
if opts.fit_method == 'nn' or opts.fit_method == 'nn_rfwrapper':
    try:
        import RIFT.interpolators.senni as senni #only used for opts.fit_method == 'nn_rfwrapper' & opts.fit_method == 'nn'
        senni_ok = True #never used
        if feedback: print(" Imported: senni")
    except:
        print( " No senni ") #don't get this output
        senni_ok = False
if opts.fit_method == 'gp_sparse':
    try:
        import RIFT.interpolators.internal_GP #this is just a test - imported again later if ok
        internalGP_ok = True #checked later to import again: only used if opts.fit_method == gp_sparse
        if feedback: print(" Import check: internal_GP OK")
    except:
...
if opts.fit_method == 'gp-torch':
    try:
        ...
        gpytorch_ok = True
        if feedback: print(" Imported: gpytorch_wrapper")
    except:
        print( " No gpytorch_wrapper ") #-------- usual print
        gpytorch_ok = False
...
if opts.n_events_to_analyze > 1: #force no plots if > 1 EOS realization
    opts.no_plots = True
    no_plots = True
lnL_shift = 0
...
#eos initialization moved to loop below
...
test_converged={} #sent to sampler as-is, apparently; i.e., sampler does not test convergence
...
###
### Prior functions : a dictionary - a whole TON of over-definition but whatever
###
...
#TODO: optimize to only import for fit_gp, fit_gp_pool, fit_gp_lazy (double check for other uses)
from sklearn.gaussian_process import GaussianProcessRegressor
from sklearn.gaussian_process.kernels import PairwiseKernel,RBF, WhiteKernel, ConstantKernel as C
if feedback: print(" Imported: gaussian_process stuff from sklearn")
...
if internalGP_ok:
    from RIFT.interpolators.internal_GP import fit_gp as fit_gp_sparse #repeat import from top
else:
    def fit_gp_sparse(x):
        sys.exit(1)
...
###
### Retrieve data - literally setting up hardcode values
###
...
if not no_plots:
 import itertools #duplicate import, also on line 34 (only used here)
...
###############################################################################
### Integrate posterior - eventually
###
...
eos_name = None
n_eos_pts = 1
if opts.using_eos_for_prior and opts.using_eos: #doing this route - fake_eos overridden if suppl code has retrieve_eos func
    ...
    n_eos_pts = opts.n_events_to_analyze
elif opts.using_eos!=None and not(opts.using_eos_for_prior):
    import RIFT.physics.EOSManager as EOSManager #test if this holds to within loop...
    ...
    if eos_name.startswith("file:") and not(opts.using_eos_index is None):
        ...
        eos_dat = np.loadtxt(fname)[opts.using_eos_index:opts.using_eos_index+n_eos_pts]
        
        make_CIP_eos = True
        #prelim check for bad eos_param opt, saves milliseconds
        spec_param_array = eos_dat[0][2:]  # drop first two as lnL, sigma_lnL - will always pass w/ masses present
        ...
        my_eos = EOSManager.EOSReprimand(name=eos_name) #this will create an empty EOS object
...
if my_eos: #this doesn't trigger for externally-supplied EOS, but still true!
...
supplemental_eos=None
eos_param_names = None
...
if opts.supplementary_likelihood_factor_code and opts.supplementary_likelihood_factor_function:
  ...
  if opts.using_eos_for_prior:
          # Load in filename & all data, save for loop
          fname = opts.using_eos.replace('file:', '')
          dat = np.genfromtxt(fname,names=True)[opts.using_eos_index:opts.using_eos_index+n_eos_pts]   # Parse file for them, to reduce need for burden parsing, and avoid burden/confusion.
          eos_param_names = dat.dtype.names
          eos_dat = dat.view((float, len(eos_param_names)))
  ...
  if hasattr(external_likelihood_module,name_prep):
    supplemental_ln_likelhood_prep=getattr(external_likelihood_module,name_prep) #TODO: typo
...
#PUT ALL OF SAMPLER STUFF INTO LOOP FOR NOW
hyper_out_list = []
fname_output_integral_orig = opts.fname_output_integral
for i in np.arange(n_eos_pts):
    print("\n---------- SAMPLING FOR EOS LINE "+str(opts.using_eos_index+i)+" ----------")
    eos_dat_line = eos_dat[i]
    
    # Set up EOS
    if opts.using_eos_for_prior and opts.using_eos and supplemental_init is not None: 
        ...
        try:
            ...
            #append low-lnL val for unphysical EOS (same file format as w/ suppl prior code)
            eos_dat_line[0] = -1e6
            eos_dat_line[1] = 1.
            hyper_out_list.append(eos_dat_line)
            continue
    ...
    #if more than 1 line, update output filenames to be different
    if n_eos_pts > 1: #regular RIFT runs must not be affected by this!
        newout = ""
        try:
        ...
        if opts.fname_output_integral == opts.fname_output_samples:
    ...
    #File 1/7 - just the lnL (MARG-X-X.dat)
    #   Warning: integral_result.dat uses *original* prior, before any reweighting
    if not (opts.save_hyperfile_only) and not (opts.chunk_save):
        np.savetxt(opts.fname_output_integral+".dat", [ln_integrand_value+lnL_shift])
    #elif not(opts.save_hyperfile_only) and opts.chunk_save: handled after loop

    #File 2/7 - lnL + EOS line (MARG-X-X+annotation.dat) - hyperpipe file
    eos_extra = []
    ...
    #        eos_extra += opts.eos_param
        ...
        fname = opts.using_eos.replace('file:','')
        params_here = np.loadtxt(fname)[opts.using_eos_index+i][2:] #TODO: redundant == eos_dat_line[2:]
        linefirst =''
    ...
    if opts.chunk_save:
        if not(opts.using_eos) or not(opts.using_eos.startswith('file:')):
            ...
            hyper_out_list.append(np.float64(str_out + eos_extra)) #len = 3 + len(using_eos) + eos.spec_params
        else:
            eos_dat_here = eos_dat_line
            eos_dat_here[0] = ln_integrand_value
            eos_dat_here[1] = np.sqrt(var)/res
            hyper_out_list.append(eos_dat_here) #compile for later
    else:
    ...
    if neff < len(low_level_coord_names):
        print(" neff < # low level coords  ->  PLOTS WILL FAIL ")
    ...
    #TODO: chunk_save not supported for any of the remaining valid files (3/7, 4/7, 5/7)
    ...
    np.savetxt(opts.fname_output_integral+"+annotation_ESS.dat",[[ln_integrand_value, np.sqrt(var)/res, neff, n_ESS]],header=" lnL sigmaL neff n_ESS ") #File 3/7
    ...
    np.savetxt(opts.fname_output_integral+"_withpriorchange.dat", [log_res_reweighted])  # should agree with the usual result, if no prior changes #File 4/7
    ...
    ###########################################################################
    '''
    Much of this does not need to be in the loop:
        it will only be utilized when n = 1
    just need to keep down to sample saving (6/7, 7/7)
    will need to store sampler results that get used in the analyses below 
    (need to check what/where they are)
    
    N.B.: Last loop iteration vars remain in-scope after exiting loop
          
    Ideal: find out what gets used below, save it to vars defined before the 
    loop. (If >1 eos line, all plotting banned regardless.)
    '''
...
if not (opts.chunk_save):
    sys.exit(0) #regular RIFT exit point
...
if len(hyper_out_list) == 0:
    print("\n WARNING: no data to save! Will not produce an output file for these lines.")
    sys.exit(0)
...
hyper_out_array = np.asarray(hyper_out_list,dtype=np.float64)

#File 1/7
if not(opts.save_hyperfile_only) and opts.chunk_save:
    lnL_shifted = hyper_out_array[:,0] + lnL_shift
    np.savetxt(opts.fname_output_integral+".dat", lnL_shifted)

#File 2/7: MARG-0-0+annotation.dat
if opts.chunk_save:
    # remove invalid lines - unneeded here
    ...
    print("\nChunk file saved; length =",len(hyper_out_array))
