#!/usr/bin/env python
#
#  util_ConstructEOSPosterior.py modified for HyperPipe but without RoboCode
...
if supplemental_coordinate_convert is None:
    ...
    if supplemental_coordinate_invert is None:
        def convert_coords(x_in, _low=low_level_coord_names, _coord=coord_names):
            ...
            return supplemental_coordinate_convert(x_in, coord_names=_low)
...
if supplemental_coordinate_invert:
    #re-convert
    dat_out[:,2:] = supplemental_coordinate_invert(dat_out[:,2:], coord_names=dat_orig_names)
